...
export async function findUserByEmail(email:string){
    const result = await pool.query(`SELECT user_id, password, is_verified FROM users WHERE email = $1`,[email]);
    
    return result.rows[0] || null;
}

export async function findUserByVerificationToken(token:string){
    const result = await pool.query(`SELECT user_id FROM users WHERE verify_token = $1`,[token]);
