...
export const getUserInfo = async(req : AuthRequest,res : Response) => {
    try{
        ...
        const user = await pool.query(`SELECT name,email, profile_picture FROM users WHERE user_id = $1`,[userId]);
...
export const updateName = async(req : AuthRequest,res : Response) => {
    ...
    try{
        ...
        const user = await pool.query(`UPDATE users SET name = $1 WHERE user_id = $2 returning email,name,profile_picture`,[nickname,userId]);
...
export const updateProfilePic  = async(req : AuthRequest,res : Response) => {
    try{
        ...
        const user = await pool.query(`UPDATE users SET profile_picture = $1 WHERE user_id = $2 returning email,name,profile_picture`,[profile_pic,userId]);
...
export const deleteUser  = async(req : AuthRequest,res : Response) => {
    try{
        ...
        const user = await pool.query(`DELETE FROM users WHERE user_id = $1 RETURNING user_id`,[userId]);
